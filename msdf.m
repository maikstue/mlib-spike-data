...
%           - if ftype is 'psp', w denotes the time constant (in ms) of the exponential distribution for a kernel
%             resembling a postsynaptic potential (taken from Eshel et al. (2015), Nature): (1-exp(-t))*(exp(-t/w))
%             in hindsight, this was hardly worth the effort - plot looks almost exactly like when using an exponential kernel
% notext    suppresses text output
% 
% EXAMPLES
...
% 
% HISTORY
% Dec 4, 2015      included a new filter kernel resembling a postsynaptic potential (see Eshel et al. (2015), Nature)
% Mar 21, 2014     included a boxcar (moving average) filter kernel and delete exGaussSimple
...
switch ftype
  ...
  case 'exGauss'
    if numel(w)~=2
      disp('exGauss kernel distributions not fully specified')
      return
    end
  ...
  case 'psp'
    filtbase   = 1:min([1000 5*w]);
    filtfunc   = (1-exp(-filtbase)).*(exp(-filtbase/w));
    kernel     = filtfunc./sum(filtfunc); % set integral to 1
    dummy      = conv(sdf,kernel);
    [~,maxpos] = max(kernel);
    kernel     = [-maxpos+1:length(kernel)-maxpos;kernel]';
...
if ~notext
  ...
  ' ticks of the sdf (5th to 95th percentiles) are distorted due to filtering edge effects'])
